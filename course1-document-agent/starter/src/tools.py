...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A mathematical expression to evaluate (e.g., "2 + 2", "100 * 1.15", "(500 + 300) / 4")

        Returns:
            The result of the expression as a formatted string
        """
        try:
            # Only allow safe characters: digits, operators, parentheses, dots, spaces
            if not re.match(r'^[\d\s\+\-\*\/\(\)\.\,\%\*\*]+$', expression):
                error_msg = f"Invalid expression: only basic math operations are allowed."
                logger.log_tool_use("calculator", {"expression": expression}, {"error": error_msg})
                return error_msg

            result = eval(expression, {"__builtins__": {}}, {})
            output = str(result) #f"Result: {expression} = {result}"
